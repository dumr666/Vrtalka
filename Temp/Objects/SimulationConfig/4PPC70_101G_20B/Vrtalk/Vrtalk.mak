...
PANEL_HW_SOURCE_Vrtalk=F:/1_Automation_projects/Vrtalka/Physical/SimulationConfig/Hardware.hw 
DIS_OBJECTS_Vrtalk=$(PANEL_HW_OBJECT_Vrtalk) $(KEYMAP_OBJECTS_Vrtalk)

# KeyMapping flags
$(TEMP_PATH_Vrtalk)/dis.PS2-Keyboard.vco: $(AS_PROJECT_PATH)/Physical/SimulationConfig/4PPC70_101G_20B/VC/PS2-Keyboard.dis
	$(VCHWPP) -f "$(PANEL_HW_SOURCE_Vrtalk)" -o "$(subst .vco,.vci,$(TEMP_PATH_Vrtalk)/dis.PS2-Keyboard.vco)" -n VrtalkaHMI -d Vrtalk -pal "$(PALFILE_Vrtalk)" -c "$(AS_CONFIGURATION)" -p "$(AS_PLC)" -ptemp "$(AS_TEMP_PLC)" -B "D4.26" -L "" -hw "$(CPUHWC)" -warninglevel 2 -so $(VC_STATIC_OPTIONS_Vrtalk) -sos $(VC_STATIC_OPTIONS_Shared) -keyboard "$(AS_PROJECT_PATH)/Physical/SimulationConfig/4PPC70_101G_20B/VC/PS2-Keyboard.dis" -fp "$(AS_VC_PATH)/Firmware/V4.26.5/SG4" -prj "$(AS_PROJECT_PATH)" -apj "Vrtalka" -sfas -vcob "$(VCOBJECT_Vrtalk)"
	$(VCC) -f "$(subst .vco,.vci,$@)" -o "$@" -cv "$(AS_PROJECT_PATH)/Logical/VCShared/ControlVersion.cvinfo" -k "$(VCVK_SOURCES_Vrtalk)" $(VCCFLAGS_Vrtalk) -p Vrtalk -sfas

KEYMAP_SOURCES_Vrtalk=$(AS_PROJECT_PATH)/Physical/SimulationConfig/4PPC70_101G_20B/VC/PS2-Keyboard.dis 
KEYMAP_OBJECTS_Vrtalk=$(TEMP_PATH_Vrtalk)/dis.PS2-Keyboard.vco 
